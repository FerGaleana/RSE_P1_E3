...
int main(void)
{
	...
	uint8_t str1[] = "K64F_Client:Msj1";
	uint8_t str2[] = "K64F_Client:Msj2";
	uint8_t str3[] = "K64F_Client:Msj3";
	uint8_t str4[] = "K64F_Client:Msj4";


	printf("\n raw buffer \n");
	for (int i = 0; i < 16; ++i) {
		printf("%c", str1[i]);			// Plaintext as char
	}

	printf("\n");

	for (int i = 0; i < 16; ++i) {
		printf("%.2x", str1[i]);		// Plaintext as hex
	}

	AES_init_ctx(&ctx, key);
	AES_ECB_encrypt(&ctx, str1);

	printf("\n Encrypted buffer\n");

	for (int i = 0; i < 16; ++i) {
		printf("%.2x", str1[i]);		// Ciphertext as hex
	}

	printf("\n Decrypted buffer\n");

	AES_init_ctx(&ctx, key);
	AES_ECB_decrypt(&ctx, str1);
